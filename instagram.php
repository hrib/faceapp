            ...
            if(empty($post[1])) {
                ...
                if(empty($obj)) {
                    ...
                    if($status == 'ok') {
                        ...
                        $device_id = "android-".$guid;
                        $data = '{"share_to_facebook":"1","fb_access_token":"'.$fbtoken.'","edits":{"crop_original_size":['.$w_orig.','.$h_orig.'],"crop_zoom":'.$factor.',"crop_center":[0.0,-0.0]},"device_id":"'.$device_id.'","guid":"'.$guid.'","media_id":"'.$media_id.'","caption":"'.trim($caption).'","device_timestamp":"'.time().'","source_type":"5","filter_type":"0","extra":"{}","Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}';   
                        $sig = GenerateSignature($data);
